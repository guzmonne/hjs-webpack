...
// helper for generating default HTML with NeXt
function defaultNeXtHtml (incomingData) {
  var data = assign({
    charset: 'utf-8',
    metaViewport: true,
    html: '<div id="root"></div>',
  ...
  add('</body>')
  add('<script src="' + data.publicPath + data.nxPath + '"</script>')
  return result.join('')
}
...
HJSPlugin.prototype.apply = function (compiler) {
  ...
  compiler.plugin('emit', function (compiler, callback) {
    ...
    /* Guzmonne editing                              */
    /* --------------------------------------------- */
    context.defaultNxTemplate = function(opts) {     //
      var templateData = assign({}, context, opts)   //
      return defaultNeXtHtml(templateData)           //
    }                                                //
    /* --------------------------------------------- */
